...
public:
  ...
  public:
    ...
    {
      // on gamma 0
      if (p[0] == 0) return -(std::sin(M_PI * p[1]) * std::exp(p[1]));
      // on gamma 1
      else if (p[0] == 1) return (std::sin(M_PI * p[1]) * std::exp(p[1]+1));
    }
  };
  ...
  public:
    ...
    gradient(const Point<dim> &p, const unsigned int /*component*/ = 0) const
    {
      ...
      result[1] = result[0] + std::cos(M_PI * p[1]) * std::exp(p[0] + p[1]) * M_PI;
